 ...
 *
 *   $Id: services.c 968 2011-08-03 10:16:26Z rd235 $
 *
 */   
#include <assert.h>
#include <errno.h>
...
void service_ctl(unsigned long type, char *sender, char *destination, ...)
{
	...
	//GDEBUG(2, " ---- 1 --------->>> type %d sender %s destination %s...", type, sender, destination);
			    ...
			    char *destination, ...)
{
	if (sender && sender->name) {
		va_list ap;
		va_start(ap, destination);
		//GDEBUG(2, " ---- 2 --------->>> type %d sender %s destination %s...", type, sender, destination);
