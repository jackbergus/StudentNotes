...
pthread_mutex_t fgdebug_mutex = PTHREAD_MUTEX_INITIALIZER;
#endif
...
void fgdebug(FILE *ofile, int gdebug_level, int level, const char *file, const int line, const char *func, const char *fmt, ...)
{
	va_list ap;
	if(level == 8)return;
	if (gdebug_level >= level || level == 'g' || level == 'y' || level == 'r')
	{
		va_start(ap, fmt);
#ifdef _PTHREAD_H
		MUTEX_LOCK(&fgdebug_mutex);
#endif
		if(level == 'g')        libc_fprintf(ofile, "\033[22;32m");
		else if (level == 'y')  libc_fprintf(ofile, "\033[01;33m");
		else if (level == 'r')  libc_fprintf(ofile, "\033[22;31m");
		...
		if(level == 'g' || level == 'y' || level == 'r') libc_fprintf(ofile, "\033[0m\n");
		else libc_fprintf(ofile, "\n");
		
#ifdef _PTHREAD_H        
		MUTEX_UNLOCK(&fgdebug_mutex);
#endif		
		va_end(ap);
        }
}
